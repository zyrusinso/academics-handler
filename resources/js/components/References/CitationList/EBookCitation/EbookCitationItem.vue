     ...
     <div class="citation-item">
        ...
        <div class="my-4">
         ...
         <div class="mt-4 update-delete-btns">
             <button class="btn update-btn" @click="updateCite(citeList.ebookId)">Update</button>
             <button class="btn delete-btn" @click="deleteCite(citeList.ebookId)">Delete</button>
         </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        deleteCite(ebookCitationId) {
            ...
            if(confirm('Are You Sure?')){
                ...
                if(localStorage.getItem('ebookCitation') === null) {
                    ...
                    storageCitation = JSON.parse(localStorage.getItem('ebookCitation'));
                }

                let filteredList = storageCitation.filter((item) => {
                    return item.ebookId !== ebookCitationId;
                });

                localStorage.setItem('ebookCitation', JSON.stringify(filteredList));
        ...
        changeCitationStyle(event) {
            ...
            this.citeList.ebookAuthors.forEach(item => {
                    let items = {"family": item.firstName,"given": item.lastName };
                    authors.push(items);
            });

            var items = [
               {"type":"book","multi":{"main":{},"_keys":{}},
               "title":this.citeList.ebookTitle,
               "edition": this.citeList.ebookEdition,
               "number-of-pages":this.citeList.ebookPages,
               "original-publisher-place": this.citeList.ebookPublishedCity,
               "publisher": this.citeList.ebookPublisher,
               "URL": this.citeList.ebookUrl,
               "author": authors,
               "issued":{"date-parts":[[this.citeList.ebookPublishedYear]]},
               "accessed":{"date-parts":[[this.citeList.ebookAccessYear,this.citeList.ebookAccessMonth,this.citeList.ebookAccessDay]]},
               "id":this.citeList.ebookId}
            ];
