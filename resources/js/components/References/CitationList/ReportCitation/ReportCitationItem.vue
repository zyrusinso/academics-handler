     ...
     <div class="citation-item">
        ...
        <div class="my-4">
         ...
         <div class="mt-4 update-delete-btns">
             <button class="btn update-btn" @click="updateCite(citeList.reportId)">Update</button>
             <button class="btn delete-btn" @click="deleteCite(citeList.reportId)">Delete</button>
         </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        deleteCite(reportCitationId) {
            ...
            if(confirm('Are You Sure?')){
                ...
                if(localStorage.getItem('reportCitation') === null) {
                    ...
                    storageCitation = JSON.parse(localStorage.getItem('reportCitation'));
                }

                let filteredList = storageCitation.filter((item) => {
                    return item.reportId !== reportCitationId;
                });

                localStorage.setItem('reportCitation', JSON.stringify(filteredList));
        ...
        changeCitationStyle(event) {
            ...
            this.citeList.reportAuthors.forEach(item => {
                    let items = {"family": item.firstName,"given": item.lastName };
                    authors.push(items);
            });

            var items = [
               {"type":"report","multi":{"main":{},"_keys":{}},
               "title":this.citeList.reportTitle,
               "container-title":this.citeList.reportSeriesTitle,
               "number-of-pages":this.citeList.reportPages,
               "original-publisher-place": this.citeList.reportPublishedCity,
               "publisher": this.citeList.reportPublisher,
               "URL": this.citeList.reportUrl,
               "author": this.citeList.reportCorpAuthor.trim() === '' ? authors : [{ "family": this.citeList.reportCorpAuthor}],
               "issued":{"date-parts":[[this.citeList.reportPublishedYear]]},
               "accessed":{"date-parts":[[this.citeList.reportAccessYear,this.citeList.reportAccessMonth,this.citeList.reportAccessDay]]},
               "id":this.citeList.reportId}
            ];
