     ...
     <div class="citation-item">
        ...
        <div class="my-4">
         ...
         <div class="mt-4 update-delete-btns">
             <button class="btn update-btn" @click="updateCite(citeList.citationId)">Update</button>
             <button class="btn delete-btn" @click="deleteCite(citeList.citationId)">Delete</button>
         </div>
    </div>
</template>
    ...
    methods: {
        deleteCite(citationId) {
            ...
            if(confirm('Are You Sure?')){
                ...
                localStorage.setItem('storageCitation', JSON.stringify(filteredList));
        ...
        changeCitationStyle(event) {
            ...
            var items = [
                {
                    "type":"webpage","multi":{"main":{},"_keys":{}},
                    "title": this.citeList.citationTitle,
                    "source": this.citeList.citationPublisher,
                    "URL": this.citeList.citationUrl,
                    "author":[{"family":this.citeList.citationFirstName,"given": this.citeList.citationLastName,"multi":{"_key":{}}}],
                    "issued":{"date-parts":[[this.citeList.citationPublishedYear,this.citeList.citationPublishedMonth, this.citeList.citationPublishedDay
                    ]]},
                    "accessed":{"date-parts":[[date.getFullYear(),date.getMonth() + 1,date.getDate()]]},
                    "id": this.citeList.citationId
                    }
            ];
