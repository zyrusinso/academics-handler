     ...
     <div class="citation-item">
        ...
        <div class="my-4">
         ...
         <div class="mt-4 update-delete-btns">
             <button class="btn update-btn" @click="updateCite(citeList.bookId)">Update</button>
             <button class="btn delete-btn" @click="deleteCite(citeList.bookId)">Delete</button>
         </div>
    </div>
</template>
        ...
        deleteCite(bookCitationId) {
            ...
            if(confirm('Are You Sure?')){
                ...
                if(localStorage.getItem('bookCitation') === null) {
                    ...
                    storageCitation = JSON.parse(localStorage.getItem('bookCitation'));
                }

                let filteredList = storageCitation.filter((item) => {
                    return item.bookId !== bookCitationId;
                });

                localStorage.setItem('bookCitation', JSON.stringify(filteredList));
            ...
            var items = [
                {
                    ...
                    "title": this.citeList.bookTitle,
                    "source": this.citeList.bookPublisher,
                    "author":[{"family":this.citeList.bookAuthorFirstName,"given": this.citeList.bookAuthorLastName,"multi":{"_key":{}}}],
                    "edition": this.citeList.bookEditon,
                    "number-of-pages": this.citeList.bookPages,
                    "issued":{"date-parts":[[this.citeList.bookPublished
                    ]]},
                    "accessed":{"date-parts":[[date.getFullYear(),date.getMonth() + 1,date.getDate()]]},
